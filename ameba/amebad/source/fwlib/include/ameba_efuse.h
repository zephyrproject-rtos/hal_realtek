...
#ifndef _AMEBA_EFUSE_H_
#define _AMEBA_EFUSE_H_
...
/** @defgroup EFUSE_LOGICAL_definitions
  * @{
  */
#define EFUSE_MAP_LEN			1024 /*!< logical map len in byte */
#define EFUSE_MAX_SECTION_8711B		(EFUSE_MAP_LEN >> 3) /*!< logical map len in section */
#define OTP_LMAP_LEN					EFUSE_MAP_LEN
...
__STATIC_INLINE int EFUSE_LMAP_READ(u8 *pbuf)
{
	return EFUSE_LogicalMap_Read(pbuf);
}

__STATIC_INLINE int EFUSE_LMAP_WRITE(u32 addr, u32 cnts, u8 *data)
{
	if (is_power_supply18() == TRUE) {
		RTK_LOGS(NOTAG, RTK_LOG_ERROR, "Please Switch to 3.3V to PG EFUSE !!!!!");
		//while (1);
...
__STATIC_INLINE u32 otp_logical_remain(void)
{
	u32 Idx = 0;
	u8 DataTemp0, WordEn;

...
void CMP_Init(CMP_InitTypeDef *CMP_InitStruct)
{
	...
	tmp = PLL_BASE->PLL_LPSD;
	tmp |= PLL_BIT_SD_POSEDGE;
	PLL_BASE->PLL_LPSD = tmp;
}
...
/**
  * @brief Set the comparator's comparison detection mode.
  * @param det_mode: can be one of the following values:
  *		@arg COMP_LEVEL_DETECT: Level detection mode. When the comparison criterion matches, bit_comp_wkx will send interrupt continuously.
  *		@arg COMP_EDGE_DETECT: Edge detection mode. When the comparison criterion matches, bit_comp_wkx will send interrupt once at the time of matching.
  * @retval None
  */
void CMP_SetCompMode(u8 det_mode)
{
	CMP_TypeDef *comparator = CMP_DEV;
	...
	if (COMP_LEVEL_DETECT == det_mode) {
		comparator->COMP_INTR_CTRL &= ~COMP_BIT_IT_DET_MOD;
	} else {
		comparator->COMP_INTR_CTRL |= COMP_BIT_IT_DET_MOD;
	}
}

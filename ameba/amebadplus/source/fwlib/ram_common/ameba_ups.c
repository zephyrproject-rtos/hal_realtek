...
void UPS_SrcConfig(u8 SrcType)
{
	...
	u32 Temp = HAL_READ32(SYSTEM_CTRL_BASE, REG_LSYS_UPS_CTRL);
	Temp &= ~LSYS_MASK_UPS_SRC_SEL;
	Temp |= LSYS_UPS_SRC_SEL(SrcType);
	HAL_WRITE32(SYSTEM_CTRL_BASE, REG_LSYS_UPS_CTRL, Temp);
}
...
void UPS_DstConfig(u8 DstType)
{
	...
	if (DstType == UPS_DST_NONE) {
		u32 Temp = HAL_READ32(SYSTEM_CTRL_BASE, REG_LSYS_UPS_CTRL);
		Temp &= ~(LSYS_BIT_UPS_OUT_PWM_TRIG | LSYS_BIT_UPS_OUT_TIM_EN);
		HAL_WRITE32(SYSTEM_CTRL_BASE, REG_LSYS_UPS_CTRL, Temp);
		return;
	}
	...
	if (DstType & UPS_DST_TIM_EN) {
		u32 Temp = HAL_READ32(SYSTEM_CTRL_BASE, REG_LSYS_UPS_CTRL);
		Temp |= LSYS_BIT_UPS_OUT_TIM_EN;
		HAL_WRITE32(SYSTEM_CTRL_BASE, REG_LSYS_UPS_CTRL, Temp);
	}

	if (DstType & UPS_DST_PWM_TRIG) {
		u32 Temp = HAL_READ32(SYSTEM_CTRL_BASE, REG_LSYS_UPS_CTRL);
		Temp |= LSYS_BIT_UPS_OUT_PWM_TRIG;
		HAL_WRITE32(SYSTEM_CTRL_BASE, REG_LSYS_UPS_CTRL, Temp);
	}
}
...
{
	u32 Temp = HAL_READ32(SYSTEM_CTRL_BASE, REG_LSYS_UPS_CTRL);
	Temp &= ~LSYS_MASK_UPS_DBC_CNT;
	Temp |= LSYS_UPS_DBC_CNT(Dbc_Cnt);
	HAL_WRITE32(SYSTEM_CTRL_BASE, REG_LSYS_UPS_CTRL, Temp);
}

...
/**
  ...
  * @param  NewState: new state of RTC.
...
{
	if (NewState == ENABLE) {
		u32 temp = HAL_READ32(RTC_MISC_REG_BASE, RTC_MISC_OSC131K_CTRL);
		temp |= RTC_BIT_FEN_RTC;
		HAL_WRITE32(RTC_MISC_REG_BASE, RTC_MISC_OSC131K_CTRL, temp);
	} else {
		u32 temp = HAL_READ32(RTC_MISC_REG_BASE, RTC_MISC_OSC131K_CTRL);
		temp &= ~RTC_BIT_FEN_RTC;
		HAL_WRITE32(RTC_MISC_REG_BASE, RTC_MISC_OSC131K_CTRL, temp);
...
/**
  * @brief  RTC clk source select
  * @param  CLK_Source:SDM32K or OSC131K_DIV4
  * @retval None
  */
void RTC_ClkSource_Select(u8 CLK_Source)
{
	u32 temp = HAL_READ32(RTC_MISC_REG_BASE, RTC_MISC_OSC131K_STATE);
	temp &= RTC_CKSL_RTC(CLK_Source);
	HAL_WRITE32(RTC_MISC_REG_BASE, RTC_MISC_OSC131K_STATE, temp);
}

/**
  * @brief  Get OSC131k_STATE
  * @param  None
  * @retval None
  */
...
/**
  * @brief  Set OSC131k_STATE
  * @param  temp: the value you want to write into RTC_MISC_OSC131K_STATE reg.
  * @retval None
  */
...
/**
  * @brief  Clear rtc_det_intr
  * @param  none
  * @retval None
  */
void RTC_ClearDetINT(void)
{
	HAL_WRITE32(SYSTEM_CTRL_BASE, REG_AON_RTC_DET_CTRL1, AON_BIT_RTC_DET_INTR);
}

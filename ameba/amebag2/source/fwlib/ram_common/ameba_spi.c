...
void SSI_SetRole(SPI_TypeDef *spi_dev, u32 role)
{
	...
	if (spi_dev == SPI0_DEV) {
		Temp &= ~LSYS_BIT_SPI0_MST;
		if (role == SSI_MASTER) {
			Temp |= LSYS_BIT_SPI0_MST;
		}
	} else {
		Temp &= ~LSYS_BIT_SPI1_MST;
		if (role == SSI_MASTER) {
			Temp |= LSYS_BIT_SPI1_MST;
		}
	}
...
/**
  ...
  *       If enabled, the NDF in register CTRL1 must be set.
  */
...
/**
  * @brief  Set SPIx the number of data frame to be transfered.
  * @param  spi_dev: where spi_dev can be SPI0_DEV or SPI1_DEV.
  * @param  DataFrameNumber: The number of data frames that to be transfered.
  * @retval None
  * @note Only valid for master
  ...
  */
void SSI_SetDataFrameNumber(SPI_TypeDef *spi_dev, u32 DataFrameNumber)
{
	u32 Ctrlr1Value;

	assert_param((DataFrameNumber >= 1) && (DataFrameNumber <= 0x10000));

	Ctrlr1Value = spi_dev->SPI_CTRLR1;
	Ctrlr1Value &= ~SPI_MASK_NDF;
	Ctrlr1Value |= SPI_NDF(DataFrameNumber - 1);

	SSI_Cmd(spi_dev, DISABLE);
	spi_dev->SPI_CTRLR1 = Ctrlr1Value;
	SSI_Cmd(spi_dev, ENABLE);
}

...
void ADC_StructInit(ADC_InitTypeDef *ADC_InitStruct)
{
	...
	ADC_InitStruct->ADC_SamplePeriodUs = 6;
...
void ADC_Init(ADC_InitTypeDef *ADC_InitStruct)
{
	...
	assert_param(ADC_InitStruct->ADC_SamplePeriodUs > 0x0);
	assert_param(ADC_InitStruct->ADC_SamplePeriodUs <= 0x400);
	...
	for (i = 0; i < ADC_InitStruct->ADC_CvlistLen + 1; i++) {
		value = adc->ADC_CLK_HCNTx_DIV[ADC_InitStruct->ADC_Cvlist[i]];
		value &= ~ADC_MASK_CLK_HCNTx;
		value |= ADC_CLK_HCNTx(ADC_InitStruct->ADC_SamplePeriodUs * 0x4 - 0x3);
		adc->ADC_CLK_HCNTx_DIV[ADC_InitStruct->ADC_Cvlist[i]] = value;
	}
	adc->ADC_CLK_LCNT0_DIV = ADC_CLK_LCNT(0x2);
...
/**
 * @brief Set sample period time of specified ADC channel in us.
 * @param ADC_channel This parameter can be a value of @ref ADC_Chn_Selection.
 * @param PeriodUs ADC sample period in us.
 * 		This parameter can be set to 0x1~0x40F.
 * @return None
 */
void ADC_SetChanClk(u8 ADC_channel, u16 PeriodUs)
{
	...
	u8 LCnt = ADC_GET_CLK_LCNT(adc->ADC_CLK_LCNT0_DIV);

	assert_param(PeriodUs > 0x0 && PeriodUs < 0x410);
	...
	value = adc->ADC_CLK_HCNTx_DIV[ADC_channel];
	value &= ~ADC_MASK_CLK_HCNTx;
	value |= ADC_CLK_HCNTx(ClkCnt - 0x1 - LCnt);
	adc->ADC_CLK_HCNTx_DIV[ADC_channel] = value;
}
...
void ADC_TimerTrigCmd(u8 Tim_Idx, u32 PeriodMs, u32 NewState)
{
	...
	if (NewState != DISABLE) {
		/* avoid overflow */
		assert_param(PeriodMs > 0 && PeriodMs < 131072);

		RTIM_TimeBaseStructInit(&TIM_InitStruct);
		TIM_InitStruct.TIM_Idx = Tim_Idx;
		/* ms to tick */
		TIM_InitStruct.TIM_Period = (PeriodMs * 32768) / 1000;
...
/**
 * @brief Set ADC dummy cycle time and related settle time.
 * @param DummyUs ADC dummy cycle time in us.
 * @param SettleUs Settle time for sample data from CapTouch to ADC in us.
 * @return None
 */
void ADC_SetDummyCycle(u32 DummyUs, u8 SettleUs)
{
	ADC_TypeDef *adc = ADC;
	u32 value;
	u32 DummyCnt = DummyUs * 0x4;
	u8 SettleCnt = SettleUs * 0x4;

	assert_param(DummyCnt < 0x2000000);
	assert_param(SettleCnt < 0x10);

	if (TrustZone_IsSecure()) {
		adc = ADC_S;
	}

	value = adc->ADC_DUMMY_CNT;
	value &= ~ADC_MASK_DUMMY_CHN_SCNT;
	value |= ADC_DUMMY_CHN_SCNT(DummyCnt);
	adc->ADC_DUMMY_CNT = value;

	value = adc->ADC_SETTLE;
	value &= ~ADC_MASK_SETTLE_CTC_TO_ADC;
	value |= ADC_SETTLE_CTC_TO_ADC(SettleCnt);
	adc->ADC_SETTLE = value;
}
...
void ADC_KeepPowerCmd(u32 NewState)
{
	...
	if (ENABLE == NewState) {
		adc->ADC_PWR_CTRL |= ADC_BIT_PWR_KEEP;
	} else {
		adc->ADC_PWR_CTRL &= ~ADC_BIT_PWR_KEEP;
	}
}

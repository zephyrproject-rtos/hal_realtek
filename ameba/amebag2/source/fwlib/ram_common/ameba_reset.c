...
void CHIPEN_WorkMode(enum CHIPEN_WORK_MODE mode)
{
	...
	HAL_WRITE32(SYSTEM_CTRL_BASE, REG_AON_CHIPEN_CTRL, Temp);
}
...
void CHIPEN_ClearINT(u32 INTrBit)
{
	...
	HAL_WRITE32(SYSTEM_CTRL_BASE, REG_AON_CHIPEN_ISR, Temp);
}
...
{
	u32 Temp = HAL_READ32(SYSTEM_CTRL_BASE, REG_AON_CHIPEN_ISR);
...
{
	return (HAL_READ32(SYSTEM_CTRL_BASE, REG_LSYS_BOOT_REASON_SW) & 0x0001FFFF);
}
...
void System_Reset(void)
{
	...
	HAL_WRITE32(SYSTEM_CTRL_BASE, REG_LSYS_SW_RST_TRIG, SYS_RESET_KEY);
	HAL_WRITE32(SYSTEM_CTRL_BASE, REG_LSYS_SW_RST_CTRL, Trig);
	HAL_WRITE32(SYSTEM_CTRL_BASE, REG_LSYS_SW_RST_TRIG, SYS_RESET_TRIG);

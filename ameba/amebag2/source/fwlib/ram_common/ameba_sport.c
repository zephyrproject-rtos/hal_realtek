...
void AUDIO_SP_Init(u32 index, u32 direction, SP_InitTypeDef *SP_InitStruct)
{
	...
	if (direction == SP_DIR_TX) {
		...
		SPORTx->SP_CTRL0 |= ((SP_DATA_LEN_SEL_TX_0(SP_InitStruct->SP_SelWordLen)) | (SP_DATA_FORMAT_SEL_TX(SP_InitStruct->SP_SelDataFormat)) | (SP_SEL_I2S_TX_CH(
...
/**
 * @brief  Enable or disable SPORT phase counter divider64.
 * @param  index: select SPORT.
 * @param  state: enable or disable.
 * @retval None
 */
void AUDIO_SP_SetPhaseCounterDiv64(u32 index, u32 state)
{
	AUDIO_SPORT_TypeDef *SPORTx = AUDIO_DEV_TABLE[index].SPORTx;
	if (state == ENABLE) {
...
/**
 * @brief  Enable or disable TX/RX exact phase counter,counted by SPORT clock. After enable this bit, LRCLK detect start.
 * @param  index: select SPORT.
 * @param  state: enable or disable.
 * @retval None
 */
void AUDIO_SP_SetExactPhaseCounter(u32 index, u32 state)
{
	AUDIO_SPORT_TypeDef *SPORTx = AUDIO_DEV_TABLE[index].SPORTx;
	if (state == ENABLE) {
		SPORTx->SP_CTRL2 |= SP_BIT_EX_PHASE_CNT_EN;
	} else {
		SPORTx->SP_CTRL2 &= ~SP_BIT_EX_PHASE_CNT_EN;
	}
}
...
/**
 * @brief  Enable or disable WIFI TSFT trigger to start audio stream.
 * @param  index: select SPORT.
 * @param  state: enable or disable.
 * @retval None
 */
void AUDIO_SP_SetWIFITSFTTrig(u32 index, u32 state)
{
	AUDIO_SPORT_TypeDef *SPORTx = AUDIO_DEV_TABLE[index].SPORTx;
	if (state == ENABLE) {
		SPORTx->SP_TX_LRCLK |= SP_BIT_WIFI_TRIG_AUDIO_EN;
	} else {
		SPORTx->SP_TX_LRCLK &= ~SP_BIT_WIFI_TRIG_AUDIO_EN;
	}
}
...
/**
 * @brief  Enable or disable WIFI TSFT trigger fs_phase_latch.
 * @param  index: select SPORT.
 * @param  state: enable or disable.
 * @retval None
 */
void AUDIO_SP_SetWIFITSFTTrigLatch(u32 index, u32 state)
{
	AUDIO_SPORT_TypeDef *SPORTx = AUDIO_DEV_TABLE[index].SPORTx;
	if (state == ENABLE) {
		SPORTx->SP_RX_LRCLK |= SP_BIT_EN_TSFT_TRIG_LATCH;
	} else {
		SPORTx->SP_RX_LRCLK &= ~SP_BIT_EN_TSFT_TRIG_LATCH;
	}
}
 ...
 * @param  index: select SPORT.
 * @retval tx_counter:SPORT TX counter.
 */
...
/**
 * @brief  Get SPORT Tx exact phase value, it counted by SPORT clcok(98.304M/45.1584M).
 * @param  index: select SPORT.
 * @retval tx_ex_phase:SPORT TX exact phase value.
 */
u32 AUDIO_SP_GetTXExactPhaseVal(u32 index)
{
	AUDIO_SPORT_TypeDef *SPORTx = AUDIO_DEV_TABLE[index].SPORTx;
	u32 tx_ex_phase = (SPORTx->SP_TX_EX_PHASE_RPT) & SP_MASK_TX_EX_PHASE_RPT;
	return tx_ex_phase;
}
 ...
 * @param  index: select SPORT.
 * @retval rx_counter:SPORT RX counter.
 */
...
/**
 * @brief  Get SPORT Rx exact phase value, it counted by SPORT clcok(98.304M/45.1584M).
 * @param  index: select SPORT.
 * @retval rx_ex_phase:SPORT RX exact phase value.
 */
u32 AUDIO_SP_GetRXExactPhaseVal(u32 index)
{
	AUDIO_SPORT_TypeDef *SPORTx = AUDIO_DEV_TABLE[index].SPORTx;
	u32 rx_ex_phase = (SPORTx->SP_RX_EX_PHASE_RPT) & SP_MASK_RX_EX_PHASE_RPT;
	return rx_ex_phase;
}

/**
 * @brief  Set LRCLK start edge.
 * @param  index: select SPORT.
 * @param  state: rising edge or falling edge.
 * @retval None
 */
void AUDIO_SP_SetLRCLKStartEdge(u32 index, u32 state)
{
	AUDIO_SPORT_TypeDef *SPORTx = AUDIO_DEV_TABLE[index].SPORTx;
	if (state == RISING_EDGE) {
		SPORTx->SP_CTRL2 |= SP_BIT_LRC_START_POL;
	} else {
		SPORTx->SP_CTRL2 &= ~SP_BIT_LRC_START_POL;
	}
}
...
/**
 * @brief  Enable or disable LRCLK start interrupt.
 * @param  index: select SPORT.
 * @param  state: enable or disable.
 * @retval None
 */
void AUDIO_SP_SetLRCLKStartIrq(u32 index, u32 state)
{
	AUDIO_SPORT_TypeDef *SPORTx = AUDIO_DEV_TABLE[index].SPORTx;
	if (state == ENABLE) {
		SPORTx->SP_CTRL2 |= SP_BIT_LRC_START_INTR_MASK;
	} else {
		SPORTx->SP_CTRL2 &= ~SP_BIT_LRC_START_INTR_MASK;
	}
}

/** @brief  Clear LRCLK start interrupt.
 * @param  index: select SPORT.
 * @retval None
 */
void AUDIO_SP_ClearLRCLKStartIrq(u32 index)
{
	AUDIO_SPORT_TypeDef *SPORTx = AUDIO_DEV_TABLE[index].SPORTx;
	SPORTx->SP_INT_CTRL |= SP_BIT_LRC_START_INTR;
}

/**
 * @brief  Enable or disable LRCLK stop interrupt.
 * @param  index: select SPORT.
 * @param  state: enable or disable.
 * @retval None
 */
void AUDIO_SP_SetLRCLKStopIrq(u32 index, u32 state)
{
	AUDIO_SPORT_TypeDef *SPORTx = AUDIO_DEV_TABLE[index].SPORTx;
	if (state == ENABLE) {
		SPORTx->SP_CTRL2 |= SP_BIT_LRC_STOP_INTR_MASK;
	} else {
		SPORTx->SP_CTRL2 &= ~SP_BIT_LRC_STOP_INTR_MASK;
	}
}
...
/**
 * @brief  Clear LRCLK stop interrupt.
 * @param  index: select SPORT.
 * @retval None
 */
void AUDIO_SP_ClearLRCLKStopIrq(u32 index)
{
	AUDIO_SPORT_TypeDef *SPORTx = AUDIO_DEV_TABLE[index].SPORTx;
	SPORTx->SP_INT_CTRL |= SP_BIT_LRC_STOP_INTR;
}

/**
 * @brief  Set LRCLK stop threshold.
 * @param  index: select SPORT.
 * @param  threshold: calculate it by LRCLK and SPORT clock, threshold =（sport_clk/lrclk）*1.5.
 * @retval None
 */
void AUDIO_SP_SetLRCLKStopThreshold(u32 index, u32 threshold)
{
	AUDIO_SPORT_TypeDef *SPORTx = AUDIO_DEV_TABLE[index].SPORTx;
	SPORTx->SP_CTRL2 &= ~SP_MASK_LRC_STOP_THRESH;
	SPORTx->SP_CTRL2 |= SP_LRC_STOP_THRESH(threshold);
}

/**
 * @brief  Get LRCLK stop threshold.
 * @param  index: select SPORT.
 * @retval threshold
 */
u32 AUDIO_SP_GetLRCLKStopThreshold(u32 index)
{
	AUDIO_SPORT_TypeDef *SPORTx = AUDIO_DEV_TABLE[index].SPORTx;
	u32 threshold = (SPORTx->SP_CTRL2) & SP_MASK_LRC_STOP_THRESH;
	return threshold;
}

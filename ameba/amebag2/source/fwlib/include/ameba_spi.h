...
#ifndef _AMEBA_SPI_H_
#define _AMEBA_SPI_H_
...
#define SPI_BIT_CSF_EN     ((u32)0x00000001 << 15)          /*!< R/W 0x0  CS Force Control Enable. Relevant only when the SPI is configured as a serial-master device. This bit field enable SPI to force CS signal keep low until the amount of data bits (DFS * NDF) had been sent/received. SPI master will force the CS signal low even when the TX FIFO is empty during transmission.  0: CS Force Control is disabled. 1: CS Force Control is enabled.*/
...
typedef struct {
	...
	__IO uint32_t SPI_RSVD_x[3]    ;  /*!< RESERVED LOCATION FOR FUTURE USE Register,  Address offset:0x0F4-0x0FC */
	__IO uint32_t SPI_DUMMY        ;  /*!< SPI DUMMY REGISTER,  Address offset:0x100 */
	__IO uint32_t RSVD1[641]       ;  /*!< Reserved,  Address offset:0x104-0xB07 */
	__IO uint32_t SPI_DR22         ;  /*!< DATA REGISTER 22 Register,  Address offset:0xB08 */
} SPI_TypeDef;
...
_LONG_CALL_ void SSI_SetCSForce(SPI_TypeDef *spi_dev, u32 newState);

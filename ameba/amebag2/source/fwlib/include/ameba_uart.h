...
#ifndef _AMEBA_UART_H_
#define _AMEBA_UART_H_
...
/** @defgroup IER
  ...
  * @{
  */
#define RUART_BIT_ERETI  ((u32)0x00000001 << 7) /*!< R/W 0x0  RXFIFO_EMPTY_TIMEOUT interrupt enable*/
...
/** @defgroup LSR
  ...
  * @{
  */
#define RUART_BIT_RE_TIMEOUT_INT   ((u32)0x00000001 << 13) /*!< R 0x0  This bit is only set when rxfifo empty timeout interrupt is enabled(ERETI is set) 1:Rxfifo is empty for longer than TH_RETI*/
#define RUART_BIT_RXND_INT         ((u32)0x00000001 << 12) /*!< R 0x0  This bit is only set when RX debouncer has detected noise 1: Rx path noise detected interrupt*/
#define RUART_BIT_MODEM_INT        ((u32)0x00000001 << 11) /*!< R 0x0  This bit is only set when Modem status pending  1: Modem interrupt*/
#define RUART_BIT_MONITOR_DONE_INT ((u32)0x00000001 << 10) /*!< R 0x0  This bit is only set when Rx Path Monitor Done interrupt is enabled(EMDI is set) 1: Rx path monitor done*/
#define RUART_BIT_TIMEOUT_INT      ((u32)0x00000001 << 9)  /*!< R 0x0  This bit is only set when Rx Timeout interrupt is enabled(ETOI is set) 1: at least one character in the FIFO but no character has been input to the FIFO or reading from it for the time duration, which depends on the value in register REG_RX_PATH_CTRL[31:16]*/
#define RUART_BIT_RXFIFO_INT       ((u32)0x00000001 << 8)  /*!< R 0x0  This bit is only set when Received Data Available Interrupt interrupt is enabled(ERBI is set) 1: FIFO Trigger level reached or Rx FIFO full*/
#define RUART_BIT_RXFIFO_ERR       ((u32)0x00000001 << 7)  /*!< R 0x0  Uart_rx_error, depending on the FIFO control register (FCR) bit[0] * 0: clear when the CPU reads the LSR, if there are no subsequent errors in the FIFO. * 1: parity error, overrun error, framing error or break indication in the FIFO.*/
#define RUART_BIT_TX_NOT_FULL      ((u32)0x00000001 << 6)  /*!< R 0x1  TXFIFO not full indicator*/
#define RUART_BIT_TX_EMPTY         ((u32)0x00000001 << 5)  /*!< R 0x1  TXFIFO empty indicator*/
...
/** @defgroup ICR
  ...
  * @{
  */
#define RUART_BIT_RETICF  ((u32)0x00000001 << 5) /*!< W 0x0  Rxfifo empty timeout Interrupt Clear Flag*/
...
/** @defgroup TH_RETI
  * @brief UART rxfifo empty interrupt timeout threshold
  * @{
  */
#define RUART_MASK_TH_RETI   ((u32)0x0FFFFFFF << 0)           /*!< R/W 0x0  Set rxfifo empty timeout interrupt, when rxfifo is empty for longer than TH_RETI. Timeout counter works in uart_rclk.*/
#define RUART_TH_RETI(x)     (((u32)((x) & 0x0FFFFFFF) << 0))
#define RUART_GET_TH_RETI(x) ((u32)(((x >> 0) & 0x0FFFFFFF)))
/** @} */
...
typedef struct {
	...
	__IO uint32_t TH_RETI        ;  /*!< UART RXFIFO EMPTY INTERRUPT TIMEOUT THRESHOLD Register,  Address offset:0x060 */
	__IO uint32_t DUMMY          ;  /*!< UART DUMMY REGISTER,  Address offset:0x064 */
} UART_TypeDef;

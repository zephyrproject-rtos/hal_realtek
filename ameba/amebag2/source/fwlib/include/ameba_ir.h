...
/** @defgroup IR_CLK_DIV
  ...
  * @{
  */
#define IR_MASK_CLK_DIV   ((u32)0x00000FFF << 0)           /*!< R/W 0x0  IR_CLK = IO_CLK/(1 + IR_CLK_DIV) *Tx mode: divider number to generate IrDA modulation frequency. For example: sys_clk = 40MHz, modulation_freq = 455kHz, IR_DIV_NUM = (sys_clk/modulation_freq) - 1 *Rx mode: waveform sample clock. IR_DIV_NUM = (sys_clk/sample clock) -  1 For example: sample clock = 40MHz, IR_DIV_NUM = 0; sample clock = 20MHz, IR_DIV_NUM = 1*/
...
/** @defgroup IR_TX_INT_CLR
  ...
  * @{
  */
#define IR_BIT_TX_FIFO_OVER_INT_CLR  ((u32)0x00000001 << 3) /*!< WA0   Tx FIFO overflow interrupt Write 1 to clear*/
#define IR_BIT_TX_FIFO_LEVEL_INT_CLR ((u32)0x00000001 << 2) /*!< WA0   When Tx FIFO offset <= threshold value, interrupt is triggered. Write 1 to clear*/
#define IR_BIT_TX_FIFO_EMPTY_INT_CLR ((u32)0x00000001 << 1) /*!< WA0   Tx FIFO empty interrupt Write 1 to clear*/
#define IR_BIT_TX_FIFO_CLR           ((u32)0x00000001 << 0) /*!< WA0   Write 1 to clear Tx FIFO*/
/** @} */
...
/** @defgroup IR_TX_FIFO
  ...
  * @{
  */
#define IR_BIT_TX_DATA_TYPE       ((u32)0x00000001 << 31)           /*!< W 0x0  Data type  *0: Inactive carrier (no carrier) *1: Active carrier (carrier)*/
#define IR_BIT_TX_DATA_END_FLAG   ((u32)0x00000001 << 30)           /*!< W 0x0  *0: Normal packet *1: Last packet*/
#define IR_MASK_TX_COMPENSATION   ((u32)0x00000003 << 28)           /*!< W 0x0  *0x0: IR_TX_CLK_Period = Tsys_clk * IR_CLK_DIV *0x1: IR_TX_CLK_Period = (1 + 1/2) Tsys_clk * IR_CLK_DIV *0x2: IR_TX_CLK_Period = (1 + 1/4) Tsys_clk * IR_CLK_DIV *0x3: IR_TX_CLK_Period = Tsys_clk * (IR_TX_COMPE_DIV + 1)*/
#define IR_TX_COMPENSATION(x)     (((u32)((x) & 0x00000003) << 28))
#define IR_GET_TX_COMPENSATION(x) ((u32)(((x >> 28) & 0x00000003)))
#define IR_MASK_TX_DATA_TIME      ((u32)0x0FFFFFFF << 0)            /*!< W 0x0  Real active time = (IR_TX_DATA_TIME + 1) * IR_TX_CLK_Period*/
...
#define IR_MASK_RX_FILTER_STAGETX     ((u32)0x00000007 << 21)           /*!< R/W 0x0  *0x0: Filter <= 50ns glitch *0x1: Filter <= 75ns glitch *0x2: Filter <= 100ns glitch *0x3: Filter <= 125ns glitch ...*/
...
/** @defgroup IR_RX_INT_CLR
  ...
  * @{
  */
#define IR_BIT_RX_FIFO_CLR           ((u32)0x00000001 << 8) /*!< WA0   Write 1 to clear Rx FIFO*/
#define IR_BIT_RX_FIFO_ERROR_INT_CLR ((u32)0x00000001 << 5) /*!< WA0   Rx FIFO error read interrupt Write 1 to clear*/
#define IR_BIT_RX_CNT_THR_INT_CLR    ((u32)0x00000001 << 4) /*!< WA0   Rx count threshold interrupt Write 1 to clear*/
#define IR_BIT_RX_FIFO_OF_INT_CLR    ((u32)0x00000001 << 3) /*!< WA0   Rx FIFO overflow interrupt Write 1 to clear*/
#define IR_BIT_RX_CNT_OF_INT_CLR     ((u32)0x00000001 << 2) /*!< WA0   Rx counter overflow interrupt Write 1 to clear*/
#define IR_BIT_RX_FIFO_LEVEL_INT_CLR ((u32)0x00000001 << 1) /*!< WA0   Rx FIFO level interrupt Write 1 to clear*/
#define IR_BIT_RX_FIFO_FULL_INT_CLR  ((u32)0x00000001 << 0) /*!< WA0   Rx FIFO full interrupt Write 1 to clear*/
/** @} */
...
typedef struct {
	...
	__IO uint32_t IR_DUMMY         ;  /*!< IR DUMMY REGISTER,  Address offset:0x030 */
} IR_TypeDef;

...
/** @defgroup GPIO_INT_STATUS
  ...
  * @{
  */
#define GPIO_MASK_INT_STATUS   ((u32)0xFFFFFFFF << 0)           /*!< R 0x0  Interrupt status of Portx.*/
#define GPIO_INT_STATUS(x)     (((u32)((x) & 0xFFFFFFFF) << 0))
#define GPIO_GET_INT_STATUS(x) ((u32)(((x >> 0) & 0xFFFFFFFF)))
/** @} */

/** @defgroup GPIO_INT_STATUS_RAW
  * @brief Port raw interrupt status register
  * @{
  */
#define GPIO_MASK_RAW_INT_STATUS   ((u32)0xFFFFFFFF << 0)           /*!< R 0x0  Raw interrupt status of Portx (premasking bits).*/
#define GPIO_RAW_INT_STATUS(x)     (((u32)((x) & 0xFFFFFFFF) << 0))
#define GPIO_GET_RAW_INT_STATUS(x) ((u32)(((x >> 0) & 0xFFFFFFFF)))
/** @} */

/** @defgroup GPIO_DEBOUNCE
  * @brief Port interrupt debounce register
  * @{
  */
#define GPIO_MASK_DEBOUNCE_EN   ((u32)0xFFFFFFFF << 0)           /*!< R/W 0x0  Controls whether an external signal that is the source of an interrupt needs to be debounced to remove any spurious glitches. Writing a 1 to a bit in this register enables the debouncing circuitry. A signal must be valid for two periods of an external clock before it is internally processed. 0: No debounce (default) 1: Enable debounce  This register is available only if Portx is configured to generate interrupts (GPIO_PORTA_INTR = Include (1)) and when the debounce logic is included (GPIO_DEBOUNCE = Include (1)).*/
#define GPIO_DEBOUNCE_EN(x)     (((u32)((x) & 0xFFFFFFFF) << 0))
#define GPIO_GET_DEBOUNCE_EN(x) ((u32)(((x >> 0) & 0xFFFFFFFF)))
/** @} */

/** @defgroup GPIO_INT_EOI
  * @brief Port edge type interrupt clear register
  * @{
  */
#define GPIO_MASK_INT_EOI   ((u32)0xFFFFFFFF << 0)           /*!< WA0 0x0  Controls the clearing of edge type interrupts from Portx. When a 1 is written into a corresponding bit of this register, the interrupt is cleared. All interrupts are cleared when Portx is not configured for interrupts. 0: No interrupt clear (default) 1: Clear interrupt*/
#define GPIO_INT_EOI(x)     (((u32)((x) & 0xFFFFFFFF) << 0))
#define GPIO_GET_INT_EOI(x) ((u32)(((x >> 0) & 0xFFFFFFFF)))
/** @} */

/** @defgroup GPIO_EXT_PORT
  * @brief External Port data register
  * @{
  */
#define GPIO_MASK_EXT_PORTx   ((u32)0xFFFFFFFF << 0)           /*!< R 0x0  When Portx is configured as Input, then reading this location reads the values on the signal. When the data direction of Portx is set as Output, reading this location reads the data register for Portx.*/
#define GPIO_EXT_PORTx(x)     (((u32)((x) & 0xFFFFFFFF) << 0))
#define GPIO_GET_EXT_PORTx(x) ((u32)(((x >> 0) & 0xFFFFFFFF)))
/** @} */

/** @defgroup GPIO_ITN_LS_SYNC
  * @brief Port level interrupt synchronization
  * @{
  */
#define GPIO_BIT_LEVEL_SYNC ((u32)0x00000001 << 0) /*!< R/W 0x0  Writing a 1 to this register results in all level-sensitive interrupts being synchronized to pclk_intr. 0: No synchronization to pclk_intr (default) 1: Synchronize to pclk_intr*/
/** @} */

/** @defgroup GPIO_ID_CODE
  * @brief Port GPIO ID code register
  * @{
  */
#define GPIO_MASK_ID_CODE   ((u32)0xFFFFFFFF << 0)           /*!< R 32'h1  This register is available only if Portx is configured to generate interrupts (GPIO_PORTx_INTR = Include (1)) and interrupt detection is configured to generate on both rising and falling edges of external input signal (GPIO_INT_BOTH_EDGE=Include (1)).*/
#define GPIO_ID_CODE(x)     (((u32)((x) & 0xFFFFFFFF) << 0))
#define GPIO_GET_ID_CODE(x) ((u32)(((x >> 0) & 0xFFFFFFFF)))
/** @} */

/** @defgroup GPIO_INT_BOTHEDGE
  * @brief Port both edge interrupt register
  * @{
  */
#define GPIO_MASK_INT_BOTHEDGE   ((u32)0xFFFFFFFF << 0)           /*!< R/W 0x0  Controls the edge types of interrupt that can occur on Portx. Whenever a particular bit is programmed to 1, it enables the generation of interrupt on both the rising edge and falling edge of an external input signal corresponding to that bit on Port. The values programmed in the registers gpio_intype_level and gpio_int_polarity for this particular bit are not considered when the corresponding bit of this register is set to 1. Whenever a particular bit is programmed to 0, the interrupt type depends on the value of the corresponding bits in the gpio_inttype_level and gpio_int_polarity registers. 0: Disabled. Single edge sensitive 1: Enabled. Both edge sensitive*/
#define GPIO_INT_BOTHEDGE(x)     (((u32)((x) & 0xFFFFFFFF) << 0))
#define GPIO_GET_INT_BOTHEDGE(x) ((u32)(((x >> 0) & 0xFFFFFFFF)))
/** @} */

/** @defgroup GPIO_VER_ID_CODE
  * @brief Port GPIO component version register
  * @{
  */
...
/** @brief GPIO Port
  */
typedef struct {
	__IO uint32_t GPIO_DR ;  /*!< PORT DATA REGISTER */
	__IO uint32_t GPIO_DDR;  /*!< PORT DATA DIRECTION REGISTER */
	__IO uint32_t GPIO_CTL;  /*!< PORT DATA SOURCE REGISTER */
} GPIO_Port_TypeDef;
...
typedef struct {
	GPIO_Port_TypeDef PORT              ;  /*!< GPIO IP have 1 ports,  Address offset:0x000-0x008 */
	__IO uint32_t RSVD0[9]              ;  /*!< Reserved,  Address offset:0x00C-0x02F */
	__IO uint32_t GPIO_INT_EN           ;  /*!< PORT INTERRUPT ENABLE REGISTER,  Address offset:0x030 */
	__IO uint32_t GPIO_INT_MASK         ;  /*!< PORT INTERRUPT MASK REGISTER,  Address offset:0x034 */
	__IO uint32_t GPIO_INT_TYPE         ;  /*!< PORT INTERRUPT TRIGGER TYPE REGISTER,  Address offset:0x038 */
	__IO uint32_t GPIO_INT_POLARITY     ;  /*!< PORT INTERRUPT POLARITY REGISTER,  Address offset:0x03C */
	__I  uint32_t GPIO_INT_STATUS       ;  /*!< PORT INTERRUPT STATUS REGISTER,  Address offset:0x040 */
	__I  uint32_t GPIO_INT_STATUS_RAW   ;  /*!< PORT RAW INTERRUPT STATUS REGISTER,  Address offset:0x044 */
	__IO uint32_t GPIO_DEBOUNCE         ;  /*!< PORT INTERRUPT DEBOUNCE REGISTER,  Address offset:0x048 */
	__O  uint32_t GPIO_INT_EOI          ;  /*!< PORT EDGE TYPE INTERRUPT CLEAR REGISTER,  Address offset:0x04C */
	__I  uint32_t GPIO_EXT_PORT[1]      ;  /*!< EXTERNAL PORT DATA REGISTER,  Address offset:0x050 */
	__IO uint32_t RSVD1[3]              ;  /*!< Reserved,  Address offset:0x054-0x05F */
	__IO uint32_t GPIO_ITN_LS_SYNC      ;  /*!< PORT LEVEL INTERRUPT SYNCHRONIZATION Register,  Address offset:0x060 */
	__I  uint32_t GPIO_ID_CODE          ;  /*!< PORT GPIO ID CODE REGISTER,  Address offset:0x064 */
	__IO uint32_t GPIO_INT_BOTHEDGE     ;  /*!< PORT BOTH EDGE INTERRUPT REGISTER,  Address offset:0x068 */
	__I  uint32_t GPIO_VER_ID_CODE      ;  /*!< PORT GPIO COMPONENT VERSION REGISTER,  Address offset:0x06C */
	__IO uint32_t RSVD2[2]              ;  /*!< Reserved,  Address offset:0x070-0x077 */
	__IO uint32_t GPIO_DB_DIV_CONFIG    ;  /*!< Register,  Address offset:0x078 */
	__IO uint32_t RSVD3[32]             ;  /*!< Reserved,  Address offset:0x07C-0x0FB */
	__IO uint32_t GPIO_DUMMY            ;  /*!< Register,  Address offset:0x0FC */
} GPIO_TypeDef;
...
/** @defgroup GPIO_IRQ_ENTRY_USED_MAGIC_NUMBER
  * @{
  */
...
_LONG_CALL_ void GPIO_UserUnRegIrq(u32 GPIO_Pin);
...
_LONG_CALL_ void GPIO_SetDWWrapper(u32 Option);
_LONG_CALL_ u32 GPIO_GetDWWrapper(void);

 ...
 * @param  timer_id: An identifier that is assigned to the timer being created. Typically this
 *                   would be used in the timer callback function to identify which timer expired
 *                   when the same callback function is assigned to more than one timer.
 ...
 *                  wait for the delete command to be successfully sent to the timer command queue.
 * @retval The status is RTK_SUCCESS or RTK_FAIL
 */
 ...
 *         timer, initializes the new timers internal state, and returns a handle by which the new
 ...
 * @param  wait_ms: Specifies the time that the calling task should be held in the Blocked state to
 *                  wait for the start command to be successfully sent to the timer command queue.
 * @retval The status is RTK_SUCCESS or RTK_FAIL
 */
